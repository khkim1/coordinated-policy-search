...
class GridEnv():
    ...
    def step(self, action):
        ...
        x = self._state % grid_size
        y = self._state // grid_size

        goal1_x = goal_1 % grid_size
        goal1_y = goal_1 // grid_size

        goal2_x = goal_2 % grid_size
        goal2_y = goal_2 // grid_size

        dist1 = np.sqrt((goal1_x - x)**2 + (goal1_y - y)**2) / 1000
        dist2 = np.sqrt((goal2_x - x)**2 + (goal2_y - y)**2) / 1000

        dist = np.min([dist1, dist2])

        if self._state == goal_1 or self._state == goal_2:
            reward = 100
            done = True

            '''
            elif self._state == true_goal:
                reward = 100
                done = True
            '''
